...
export default function VocabApp() {
  ...
  return (
    <main className="library-shell">
      ...
      {view.type === 'folder' && currentCard && currentFolder && <FolderView card={currentCard} folder={currentFolder} entries={data.entries.filter((entry) => entry.folder_id === currentFolder.id)} onBack={() => setView({ type: 'card', id: currentCard.id })} onAdd={() => { setEditing(null); setModal('entry'); }} onEdit={(entry) => { setEditing(entry); setModal('entry'); }} onDelete={(entry) => void mutate({ action: 'deleteEntry', id: entry.id })} onImport={() => setModal('import')} onPractice={() => openPractice(scopeFor('folder', currentFolder.id))} />}
...
function PracticeHeader({ label, index, total, accuracy, metric, onExit }: { label: string; index: number; total: number; accuracy: number; metric: string; onExit: () => void }) {
  return <header className="practice-header"><button className="practice-brand" onClick={() => { if (confirm('结束当前练习并返回？')) onExit(); }}>刻</button><div className="practice-scope"><small>正在练习</small><strong>{label}</strong></div><div className="metrics"><div><span>进度</span><strong>{index}<small> / {total}</small></strong></div><div><span>正确率</span><strong>{Math.round(accuracy * 100)}<small>%</small></strong></div><div><span>平均速度</span><strong>{metric}</strong></div></div><button className="practice-exit" onClick={() => { if (confirm('结束当前练习并返回？')) onExit(); }}>退出</button></header>;
}
